...
class VectorIndexBuilder:
    ...
    def _get_client(self) -> chromadb.Client:
        ...
        if self.client is None:
            logger.info(f"Creating Chroma client with persistence at {self.db_path}")
            ...
            logger.info("Chroma client created successfully")
    ...
    def _get_collection(self) -> chromadb.Collection:
        ...
        if self.collection is None:
            ...
            try:
                ...
                # Use custom embedding function (we'll provide embeddings ourselves)
...
def test_vector_index():
    ...
    print("Testing Chroma vector index...")
    ...
    print(f"✅ Search results for 'urgent payment account':")
    for i, (doc, metadata, distance) in enumerate(zip(
        search_results['results']['documents'][0],
        search_results['results']['metadatas'][0], 
        search_results['results']['distances'][0]
    )):
        print(f"  {i+1}. Subject: {metadata['subject']} (distance: {distance:.3f})")
        print(f"     Category: {metadata['category']}, Phishing: {metadata['is_phishing']}")
    ...
    print(f"✅ Phishing-only search results: {phishing_results['n_results']} found")
    
    # Get stats
    db_stats = builder.get_stats()
    print(f"✅ Database stats: {db_stats}")
    
    print("✅ Vector index test completed successfully!")
