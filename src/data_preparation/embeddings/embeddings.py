...
try:
    ...
    # Fallback for direct execution when running with PYTHONPATH=src or repository root
    import sys
    from pathlib import Path

    root_path = Path(__file__).parent.parent.parent  # points to src/
    if str(root_path) not in sys.path:
        sys.path.append(str(root_path))
    try:
        # Import without 'src.' prefix to work when src/ is sys.path root
        from shared.config import Config  # type: ignore
    except ImportError:
        # Last resort: attempt prefixed form if running from repo root where src is a package directory
        try:
            from src.shared.config import Config  # type: ignore
        except ImportError as e:
            raise ImportError(
                f"Could not resolve Config module via shared.config or src.shared.config: {e}"
            )
...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        self._max_memory_cache = (
            1000  # WHY: Balance memory usage vs cache hits for typical datasets
        )

        # Thread-safe, non-blocking cache access
        self._cache_lock = threading.RLock()  # Reentrant lock for nested operations
        self._cache_stats_lock = threading.Lock()  # Separate lock for statistics
    ...
    def _load_from_cache(self, cache_key: str) -> Optional[np.ndarray]:
        """
        Load embeddings from cache with thread-safe, non-blocking access.

        Returns cached embedding if available, None otherwise.
        Uses try-lock to avoid blocking - skips cache if busy.
        """
        ...
        if not self._cache_lock.acquire(blocking=False):
            ...
            logger.debug(f"Cache busy for key: {cache_key[:8]}, proceeding without cache")
            return None
    ...
    def _save_to_cache(self, cache_key: str, embedding: np.ndarray) -> None:
        """
        Save embeddings to cache with thread-safe, non-blocking access.

        Uses try-lock to avoid blocking - skips caching if busy.
        """
        # Try to acquire lock without blocking
        if not self._cache_lock.acquire(blocking=False):
            # Cache is busy, skip caching (fail-safe)
            logger.debug(f"Cache busy for storing key: {cache_key[:8]}, skipping cache")
            return
        ...
        try:
            ...
            if len(self._memory_cache) < self._max_memory_cache:
                self._memory_cache[cache_key] = embedding
                logger.debug(f"Saved to memory cache: {cache_key[:8]}")
    ...
    def embed_batch(self, texts: List[str], batch_size: int = 32) -> List[np.ndarray]:
        ...
        if uncached_texts:
            ...
            new_embeddings = model.encode(
                uncached_texts, batch_size=batch_size, convert_to_numpy=True
            )
    ...
    def clear_memory_cache(self) -> None:
        """Clear the in-memory cache to free memory with thread-safe access."""
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_memory_cache_size": self._max_memory_cache,
        }


def test_embeddings():
    """Test function to verify embeddings work correctly."""
    logger.info("Testing sentence-transformers embeddings...")
    ...
    logger.info(f"Similarity between similar emails: {similarity_score:.3f}")
    ...
    logger.info(f"Similarity between different emails: {similarity_score2:.3f}")

    # Cache stats
    stats = generator.get_cache_stats()
    logger.info(f"Cache stats: {stats}")
    logger.info("✅ Embeddings test completed successfully!")


if __name__ == "__main__":
    # Only run the test harness when this module is executed directly
    test_embeddings()
