...
class EmailGenerator:
    ...
    def generate_legitimate_email(self) -> SimpleEmail:
        ...
        sender_email = sender_pattern.format(
            first_name=first_name.lower(), last_name=last_name.lower(), domain=domain
        )
    ...
    def generate_phishing_email(self, phishing_type: str = None) -> SimpleEmail:
        ...
        subject = subject_template.format(
            ...
            executive_name=(
                random.choice(self.executives) if phishing_type == "executive_impersonation" else ""
            ),
        )
    ...
    def generate_dataset(
        self, total_emails: int = 200, phishing_ratio: float = 0.3
    ) -> List[SimpleEmail]:
        ...
        logger.info(
            f"Generating {total_emails} emails ({num_legitimate} legitimate, {num_phishing} phishing)..."
        )
    ...
    def save_to_csv(self, emails: List[SimpleEmail], output_path: str = None) -> str:
        ...
        logger.info(
            f"   Legitimate: {len(df[df['is_phishing'] == False])} ({len(df[df['is_phishing'] == False])/len(df)*100:.1f}%)"
        )
        logger.info(
            f"   Phishing: {len(df[df['is_phishing'] == True])} ({len(df[df['is_phishing'] == True])/len(df)*100:.1f}%)"
        )
...
def main():
    ...
    try:
        ...
        logger.info(f"\n📊 Dataset Validation:")
        logger.info(
            f"   Total emails: {len(emails)} ({'✅' if len(emails) >= 150 else '❌'} >= 150)"
        )
...
# --- Compatibility Layer for Phase 8.1 Example Script ---
# The examples/implement_queries.py script was authored against an earlier
# design that expected a `ThreatEmailGenerator` with plural generation
# methods and a broader pattern taxonomy. The core generator implemented
# here is `EmailGenerator` with singular methods and four phishing types.
#
# To avoid refactoring the example script (and preserve its semantics),
# we provide a lightweight compatibility wrapper that maps the requested
# pattern names to existing phishing types and applies minor augmentations
# (attachments, timestamps, subject tweaks) so downstream threat feature
# extraction still has varied signals to work with.
#
# NOTE: This is intentionally minimal. For full fidelity, extend
# `EmailGenerator` with richer templates per pattern and update
# threat analysis modules accordingly.


class ThreatEmailGenerator:
    """Compatibility wrapper exposing the interface expected by
    examples/implement_queries.py.

    Methods:
        generate_phishing_emails(count, patterns): returns a list of SimpleEmail
        generate_legitimate_emails(count): returns a list of SimpleEmail
    """

    # Map extended pattern names -> underlying base phishing types
    _pattern_map = {
        "urgent_payment": "urgent_payment",
        "new_domain": "urgent_payment",
        "malicious_attachment": "urgent_payment",
        "executive_impersonation": "executive_impersonation",
        "wire_transfer_urgency": "urgent_payment",
        "credential_harvesting": "credential_harvest",
        "account_suspension": "account_suspension",
        "invoice_fraud": "urgent_payment",
        "hr_scam": "urgent_payment",
        "gift_card_scam": "urgent_payment",
        "domain_spoofing": "account_suspension",
        "off_hours_payment": "urgent_payment",
    }

    def __init__(self):
        self._base = EmailGenerator(SimpleConfig.from_env())

    def generate_phishing_emails(self, count: int, patterns: List[str]) -> List[SimpleEmail]:
        emails: List[SimpleEmail] = []
        if not patterns:
            patterns = ["urgent_payment"]
        for i in range(count):
            pattern = patterns[i % len(patterns)]
            phishing_type = self._pattern_map.get(pattern, "urgent_payment")
            email = self._base.generate_phishing_email(phishing_type)

            # Augment attributes to better reflect requested pattern nuances
            if pattern == "off_hours_payment":
                email.timestamp = self._base._generate_outside_hours_timestamp()
            if pattern == "malicious_attachment" and not email.attachments:
                email.attachments = ["invoice_urgent.exe"]
                email.attachment_count = 1
            if pattern == "domain_spoofing":
                # Ensure suspicious / typosquatted domain usage
                email.sender_domain = random.choice(self._base.suspicious_domains)
                # Slight tweak to subject to hint at brand impersonation
                email.subject = f"Security Notice for {email.sender_domain}"[:120]
            if pattern == "gift_card_scam":
                email.subject = "URGENT: Gift Card Purchase Required"
                email.body += "\n\nPlease acquire 10 gift cards immediately and send the codes."
            if pattern == "invoice_fraud":
                email.subject = "RE: Outstanding Invoice Discrepancy"
            if pattern == "hr_scam":
                email.subject = "CONFIDENTIAL: Salary Adjustment Review"
            if pattern == "wire_transfer_urgency":
                email.subject = "Immediate Wire Transfer Authorization Needed"

            # Preserve original pattern for downstream logic / analysis
            email.phishing_type = pattern
            emails.append(email)
        return emails

    def generate_legitimate_emails(self, count: int) -> List[SimpleEmail]:
        return [self._base.generate_legitimate_email() for _ in range(count)]
